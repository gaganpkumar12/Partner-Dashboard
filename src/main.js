...
// ─── Sync Quota (server-enforced) ────────────────────────────
async function fetchAndUpdateSyncBtn() {
  const btn = document.getElementById("refreshBtn");
  if (!btn) return;
  try {
    const quota = await client.mutation(api.mutations.getSyncQuota, {});
    const { remaining, limit } = quota;
    if (remaining <= 0) {
      btn.disabled = true;
      btn.title = `Daily sync limit reached (${limit}/${limit}). Resets at midnight IST.`;
    ...
    // If quota check fails, allow sync (fail open)
    btn.disabled = false;
    btn.title = "";
...
window.triggerSync = async function () {
  ...
  try {
    const result = await client.action(api.zohoSync.syncFromZoho, { scheduled: false });
    if (result.quotaExceeded) {
      alert(result.error); // "Daily sync limit reached..."
